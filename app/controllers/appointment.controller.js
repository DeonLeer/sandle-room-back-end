...
exports.getAppointments = (req, res) => {
  // console.log(req.userId, req.roles)
	...
	const query = `
    SELECT appointments.id, appointments.startTime, appointments.endTime, appointments.title, users.firstName, users.lastName FROM appointments
    JOIN users ON appointments.${isAdmin ? "client" : "host"}UserId = users.id
    WHERE ${isAdmin ? "host" : "client"}UserId = ${userId} ${(year && month) ? `AND appointments.startTime LIKE '${year}-${month}%'` : ''}
  `;
  console.log(query)
